...
export function Environment(name: string): BaseURL {
    return `https://${name}-screengraph-ovzi.encr.app`
}
...
export namespace run {
    ...
    export class ServiceClient {
        ...
        public async cancel(id: string): Promise<CancelRunResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("POST", `/run/${encodeURIComponent(id)}/cancel`)
            return await resp.json() as CancelRunResponse
        }
        ...
        public async start(params: StartRunRequest): Promise<StartRunResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("POST", `/run`, JSON.stringify(params))
            return await resp.json() as StartRunResponse
        }

        public async stream(id: string, params: StreamHandshake): Promise<StreamIn<RunEventMessage>> {
            // Convert our params into the objects we need for the request
            const query = makeRecord<string, string | string[]>({
                lastEventSeq: params.lastEventSeq === undefined ? undefined : String(params.lastEventSeq),
            })

            return await this.baseClient.createStreamIn(`/run/${encodeURIComponent(id)}/stream`, {query})
        }
    }
}
...
export namespace steering {
    ...
    export class ServiceClient {
        ...
        public async getDoc(category: string, filename: string): Promise<GetDocResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("GET", `/steering/docs/${encodeURIComponent(category)}/${encodeURIComponent(filename)}`)
            return await resp.json() as GetDocResponse
        }

        public async listDocs(): Promise<ListDocsResponse> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("GET", `/steering/docs`)
            return await resp.json() as ListDocsResponse
        }

        public async updateDoc(category: string, filename: string, params: UpdateDocParams): Promise<UpdateDocResponse> {
            // Now make the actual call to the API
