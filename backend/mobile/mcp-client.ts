...
export class MobileMCPClient {
  ...
  async initialize(): Promise<void> {
    ...
    this.process.on("exit", (code: number | null) => {
      ...
      this.cleanup(); // Clean up state (responseHandlers, buffer, initialized flag)
    });
    ...
    this.logger.info("mobile-mcp initialized", {
      ...
      serverName: initResponse.serverInfo?.name ?? "mobile-mcp",
    });
  ...
  async listDevices(): Promise<DeviceInfo[]> {
    const result = await this.invokeTool<string>("mobile_list_available_devices", { noParams: {} });
