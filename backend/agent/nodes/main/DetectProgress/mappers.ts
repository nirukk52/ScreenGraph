 ...
 */
export function buildDetectProgressInput(
  state: AgentState,
  _ctx: AgentContext,
): DetectProgressInput {
  ...
  const isNewScreenDiscovered =
    state.counters.screensNew > 0 && state.graphPersistenceOutcomeId !== null;
 ...
 */
export function applyDetectProgressOutput(
  prev: AgentState,
  ...
  return {
    ...
    progressEvaluationId: output.progressEvaluation
      ? `progress-${prev.runId}-${prev.stepOrdinal}`
      : null,
  };
}
